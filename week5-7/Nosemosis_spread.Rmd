...
parameters.spring <- c(b=500, κ=8000, σ1=0.25, σ2=1.5, η0=0, η1=0, ϕ0=0.08511, ϕ1=0.16936, α=0.55, γ=0.2061, δ=0.006570, α_t=0.14, λ=10000, n=2, p=1, 𝜉=8)
parameters.summer <- c(b=1500, κ=12000, σ1=0.25, σ2=1.5, η0=0, η1=0, ϕ0=0.08511, ϕ1=0.16936, α=0.12, γ=0.2835, δ=0.023300, α_t=0.14, λ=10000, n=2, p=0, 𝜉=8)
parameters.fall <- c(b=500, κ=8000, σ1=0.25, σ2=1.5, η0=0, η1=0, ϕ0=0.08511, ϕ1=0.16936, α=0.24, γ=0.2527, δ=0.015683, α_t=0.14, λ=10000, n=2, p=0, 𝜉=8)
parameters.winter <- c(b=0, κ=6000, σ1=0, σ2=1.5, η0=0.00649, η1=0.00649, ϕ0=0, ϕ1=0, α=0, γ=0, δ=0, α_t=0.14, λ=10000, n=2, p=0, 𝜉=8)
...
ev_simulation <- function (p, apiary, Ek){
  ...
  dEk <- max((1 - length(apiary$get(k)) * p) * Ek[k] + p * sum(Ek))[2]
  Ek_updated[k] <- dEk
  k <- k+1
  }
  return(Ek_updated)
...
while (index < 30) {
  ...
  while (index2 <= length(Ek)){
    if (index == 1) {
      ...
      winter.state <- c(Hi0=hives.state[(index - 2) * 5 + 1, index2], Hi1=hives.state[(index - 2) * 5 + 2, index2], Fi0=hives.state[(index - 2) * 5 + 3, index2], Fi1=hives.state[(index - 2) * 5 + 4, index2], Ei=hives.ek[index-1,])
    }
    out.year <- as.data.frame(gen_formula(winter.state))
    out.year$time <- out.year$time + 364 * index - 1
    out.years <- rbind(out.years, out.year)
    ...
    Ek[index2] <- out.year[nrow(out.year), 6]
